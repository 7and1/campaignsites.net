...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`case_studies_highlights\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`case_studies_highlights_order_idx\` ON \`case_studies_highlights\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`case_studies_highlights_parent_id_idx\` ON \`case_studies_highlights\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`case_studies_platform\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`case_studies_platform_order_idx\` ON \`case_studies_platform\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`case_studies_platform_parent_idx\` ON \`case_studies_platform\` (\`parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`case_studies_rels_tools_id_idx\` ON \`case_studies_rels\` (\`tools_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`posts\` (
  	...
  	\`published_date\` text,
  	\`category\` text,
  	\`affiliate_disclosure\` integer DEFAULT true,
  	\`meta_title\` text,
  	\`meta_description\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE INDEX \`posts_rels_tools_id_idx\` ON \`posts_rels\` (\`tools_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`case_studies_highlights\`;`)
  await db.run(sql`DROP TABLE \`case_studies_platform\`;`)
